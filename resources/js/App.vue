        ...
        <div class="p-3 border-b scry-border">
          ...
          >
            <option v-for="conn in connectionStore.availableConnections" :key="conn" :value="conn">
              {{ conn }} ({{ conn === 'pgsql' ? 'PostgreSQL' : (conn === 'mysql' ? 'MySQL' : (conn === 'mariadb' ? 'MariaDB' : (conn === 'sqlite' ? 'SQLite' : (conn === 'sqlsrv' ? 'SQL Server' : (connectionStore.driver ? connectionStore.driver.toUpperCase() : conn))))) }})
            </option>
          </select>
        </div>
